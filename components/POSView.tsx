...
const POSView: React.FC<POSViewProps> = ({ 
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full overflow-hidden bg-surface relative">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <div className="bg-white p-4 shadow-sm z-20 sticky top-0 flex flex-col md:flex-row gap-4 justify-between items-center">
            <div className="flex items-center gap-4">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">{settings.restaurantName}</h1>
        ...
        <div className="flex-1 overflow-y-auto p-4 no-scrollbar sm:custom-scrollbar">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 pb-10">
                {filteredItems.map(item => (
                  <div key={item.id} className="bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all cursor-pointer overflow-hidden group flex flex-col border border-gray-100 hover:border-primary/40" onClick={() => initiateAddToCart(item)}>
                    ...
                    <div className="p-3 flex flex-col flex-1 justify-between min-h-[80px]">
                      ...
                      <p className="font-urdu text-primary text-[11px] font-bold text-right truncate">{item.urduName}</p>
                    </div>
                  </div>
                ))}
            </div>
      ...
      {/* Cart Panel */}
      <div className="w-full lg:w-[400px] bg-white border-l flex flex-col h-[50vh] lg:h-full shadow-2xl relative z-40">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
          <div className="flex justify-between items-center border-b pb-2">
            ...
            {cart.length > 0 && <button onClick={() => setCart([])} className="text-xs text-red-500 font-bold">Clear</button>}
          </div>
            ...
            <div key={`${item.id}-${index}`} className="flex gap-3 border-b border-dashed border-gray-100 pb-3 group">
                <div className="flex-1">
                  ...
                  <div className="flex items-center mt-2 gap-3">
                      <button onClick={() => setCart(prev => prev.map((it, idx) => idx === index ? {...it, quantity: Math.max(1, it.quantity - 1)} : it))} className="w-6 h-6 rounded bg-gray-100 flex items-center justify-center text-gray-600"><Minus size={12}/></button>
                      <span className="text-xs font-bold w-4 text-center">{item.quantity}</span>
                      <button onClick={() => setCart(prev => prev.map((it, idx) => idx === index ? {...it, quantity: it.quantity + 1} : it))} className="w-6 h-6 rounded bg-gray-100 flex items-center justify-center text-gray-600"><Plus size={12}/></button>
                  </div>
                </div>
        ...
        <div className="p-4 bg-gray-50 border-t border-gray-200">
          <div className="space-y-1 mb-4 text-xs font-bold">
            ...
            {serviceCharge > 0 && <div className="flex justify-between text-gray-400"><span>Service Charge</span><span>Rs. {serviceCharge.toFixed(0)}</span></div>}
          ...
          <button disabled={cart.length === 0} onClick={() => setIsCheckingOut(true)} className="w-full py-4 bg-primary text-white rounded-2xl font-bold shadow-lg disabled:bg-gray-300 transition-all flex justify-center items-center gap-2">
      ...
      {isCheckingOut && (
          <div className="absolute inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-white rounded-3xl w-full max-w-sm p-8 shadow-2xl animate-scale-up">
                  <h3 className="text-2xl font-bold text-center mb-6">Select Payment</h3>
                  <div className="space-y-3 mb-8">
                      <button onClick={() => handleCheckout(PaymentMethod.CASH)} className="w-full py-4 bg-green-600 text-white rounded-2xl font-bold flex items-center justify-center gap-3 shadow-lg">
                          <Banknote size={24}/> Cash Payment
                      </button>
                      <button onClick={() => handleCheckout(PaymentMethod.CARD)} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-bold flex items-center justify-center gap-3 shadow-lg">
                          <CreditCard size={24}/> Card Payment
                      </button>
                      <button onClick={() => setIsCheckingOut(false)} className="w-full py-3 text-gray-500 font-bold">Back</button>
                  </div>
              </div>
          </div>
      )}
